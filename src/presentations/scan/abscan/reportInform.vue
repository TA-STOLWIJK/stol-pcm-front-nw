...
import SheetText from '@/json/scan/abscanInf.json'
...
export default {
  ...
  methods: {
    ...
    createPowerPoint() {
      ...
      if (this.dataScan.sl_j > 50) {
        rowC.push([
          { text: this.sheetText['samen'].text_x },
          {
            text: this.sheetText['samen'].text_y
          },
          {
            text: this.dataScan.sl_j,
            options: { align: 'center', color: '00B050' }
          }
        ])
      } else if (this.dataScan.sl_j < -50) {
        rowC.push([
          { text: this.sheetText['samen'].text_x },
          {
            text: this.sheetText['samen'].text_y
      ...
      if (this.dataScan.sl_k > 50) {
        rowC.push([
          { text: this.sheetText['samen'].text_z },
          {
            text: this.sheetText['samen'].text_za
          },
          {
            text: this.dataScan.sl_k,
            options: { align: 'center', color: '00B050' }
          }
        ])
      } else if (this.dataScan.sl_k < -50) {
        rowC.push([
          { text: this.sheetText['samen'].text_z },
          {
            text: this.sheetText['samen'].text_za
      ...
      if (this.dataScan.sl_l > 50) {
        rowC.push([
          { text: this.sheetText['samen'].text_zb },
          {
            text: this.sheetText['samen'].text_zc
          },
          {
            text: this.dataScan.sl_l,
            options: { align: 'center', color: '00B050' }
          }
        ])
      } else if (this.dataScan.sl_l < -50) {
        rowC.push([
          { text: this.sheetText['samen'].text_zb },
          {
            text: this.sheetText['samen'].text_zc
      ...
      if (this.dataScan.sl_m > 50) {
        rowC.push([
          { text: this.sheetText['samen'].text_zd },
          {
            text: this.sheetText['samen'].text_ze
          },
          {
            text: this.dataScan.sl_m,
            options: { align: 'center', color: '00B050' }
          }
        ])
      } else if (this.dataScan.sl_m < -50) {
        rowC.push([
          { text: this.sheetText['samen'].text_zd },
          {
            text: this.sheetText['samen'].text_ze
      ...
      let slideStatJ = pres.addSlide('P_SLIDE_Matrix')
      slideStatJ.addText(
        [{ text: this.sheetText['samen'].text_x, options: this.fontHeader }],
        this.placeHeader
      )
      let rowM = []
      rowM.push([{ text: this.sheetText['samen'].text_y, options: { bold: true } }])
      rowM.push([{ text: '' }])
      rowM.push([{ text: this.sheetText['alg'].text_a + this.dataScan.sl_j }])
      rowM.push([{ text: '' }])
      rowM.push([{ text: this.sheetText['alg'].text_b }])
      rowM.push([{ text: '' }])
      rowM.push([{ text: '' }])
      rowM.push([{ text: '' }])
      rowM.push([{ text: '' }])
      rowM.push([{ text: this.sheetText['alg'].text_c }])
      rowM.push([{ text: this.sheetText['alg'].text_d }])
      rowM.push([{ text: this.sheetText['alg'].text_e }])
      rowM.push([{ text: this.sheetText['alg'].text_f }])
      slideStatJ.addTable(rowM, this.placeTableBlock)
      if (this.dataScan.sl_j > 50) {
        slideStatJ.addImage({
          path: '../img/oScan/matrixDone.png',
          placeholder: 'Photo',
      ...
      let slideStatK = pres.addSlide('P_SLIDE_Matrix')
      slideStatK.addText(
        [{ text: this.sheetText['samen'].text_z, options: this.fontHeader }],
        this.placeHeader
      )
      let rowN = []
      rowN.push([{ text: this.sheetText['samen'].text_za, options: { bold: true } }])
      rowN.push([{ text: '' }])
      rowN.push([{ text: this.sheetText['alg'].text_a + this.dataScan.sl_k }])
      rowN.push([{ text: '' }])
      rowN.push([{ text: this.sheetText['alg'].text_b }])
      rowN.push([{ text: '' }])
      rowN.push([{ text: '' }])
      rowN.push([{ text: '' }])
      rowN.push([{ text: '' }])
      rowN.push([{ text: this.sheetText['alg'].text_c }])
      rowN.push([{ text: this.sheetText['alg'].text_d }])
      rowN.push([{ text: this.sheetText['alg'].text_e }])
      rowN.push([{ text: this.sheetText['alg'].text_f }])
      slideStatK.addTable(rowN, this.placeTableBlock)
      if (this.dataScan.sl_k > 50) {
        slideStatK.addImage({
          path: '../img/oScan/matrixDone.png',
          placeholder: 'Photo',
      ...
      let slideStatL = pres.addSlide('P_SLIDE_Matrix')
      slideStatL.addText(
        [{ text: this.sheetText['samen'].text_zb, options: this.fontHeader }],
        this.placeHeader
      )
      let rowO = []
      rowO.push([{ text: this.sheetText['samen'].text_zc, options: { bold: true } }])
      rowO.push([{ text: '' }])
      rowO.push([{ text: this.sheetText['alg'].text_a + this.dataScan.sl_l }])
      rowO.push([{ text: '' }])
      rowO.push([{ text: this.sheetText['alg'].text_b }])
      rowO.push([{ text: '' }])
      rowO.push([{ text: '' }])
      rowO.push([{ text: '' }])
      rowO.push([{ text: '' }])
      rowO.push([{ text: this.sheetText['alg'].text_c }])
      rowO.push([{ text: this.sheetText['alg'].text_d }])
      rowO.push([{ text: this.sheetText['alg'].text_e }])
      rowO.push([{ text: this.sheetText['alg'].text_f }])
      slideStatL.addTable(rowO, this.placeTableBlock)
      if (this.dataScan.sl_l > 50) {
        slideStatL.addImage({
          path: '../img/oScan/matrixDone.png',
          placeholder: 'Photo',
      ...
      let slideStatM = pres.addSlide('P_SLIDE_Matrix')
      slideStatM.addText(
        [{ text: this.sheetText['samen'].text_zd, options: this.fontHeader }],
        this.placeHeader
      )
      let rowP = []
      rowP.push([{ text: this.sheetText['samen'].text_ze, options: { bold: true } }])
      rowP.push([{ text: '' }])
      rowP.push([{ text: this.sheetText['alg'].text_a + this.dataScan.sl_m }])
      rowP.push([{ text: '' }])
      rowP.push([{ text: this.sheetText['alg'].text_b }])
      rowP.push([{ text: '' }])
      rowP.push([{ text: '' }])
      rowP.push([{ text: '' }])
      rowP.push([{ text: '' }])
      rowP.push([{ text: this.sheetText['alg'].text_c }])
      rowP.push([{ text: this.sheetText['alg'].text_d }])
      rowP.push([{ text: this.sheetText['alg'].text_e }])
      rowP.push([{ text: this.sheetText['alg'].text_f }])
      slideStatM.addTable(rowP, this.placeTableBlock)
      if (this.dataScan.sl_m > 50) {
        slideStatM.addImage({
          path: '../img/oScan/matrixDone.png',
          placeholder: 'Photo',
