...
import ReportFormal from '@/presentations/scan/hscan/reportFormal.vue'
import ReportInformal from '@/presentations/scan/hscan/reportInformal.vue'
...
import editScan from '@/views/scans/hscan/editScan.vue'
...
const resetData = async () => {
  useConfirmBeforeDelete(
    async () => {
      await scanStore.setDefaultH(route.params.id)
      await scanStore.getScanH(route.params.id)
    },
...
<template>
  <PageDefaultLayoutTwo>
    ...
    <HeaderOne
      >QuickScan exit planning {{ clientSingleStore.first_name_client }}
