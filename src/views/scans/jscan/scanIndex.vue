...
import ReportFormal from '@/presentations/scan/jscan/reportFormal.vue'
import ReportInformal from '@/presentations/scan/jscan/reportInformal.vue'
...
import editScan from '@/views/scans/jscan/editScan.vue'
...
const resetData = async () => {
  useConfirmBeforeDelete(
    async () => {
      await scanStore.setDefaultJ(route.params.id)
      await scanStore.getScanJ(route.params.id)
    },
...
<template>
  <PageDefaultLayoutTwo>
    ...
    <HeaderOne
      >QuickScan digitale strategie {{ clientSingleStore.first_name_client }}
