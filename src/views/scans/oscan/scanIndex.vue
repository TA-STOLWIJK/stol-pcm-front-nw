...
import ReportFormal from '@/presentations/scan/oscan/reportForm.vue'
import ReportInformal from '@/presentations/scan/oscan/reportInform.vue'
...
import editScan from '@/views/scans/oscan/editScan.vue'
...
const resetData = async () => {
  useConfirmBeforeDelete(
    async () => {
      await scanStore.setDefaultO(route.params.id)
      await scanStore.getScanO(route.params.id)
    },
...
<template>
  <PageDefaultLayoutTwo>
    ...
    <HeaderOne
      >QuickScan pensioen {{ clientSingleStore.first_name_client }}
