...
import ReportFormal from '@/presentations/scan/kscan/reportForm.vue'
import ReportInformal from '@/presentations/scan/kscan/reportInform.vue'
...
import editScan from '@/views/scans/kscan/editScan.vue'
...
const resetData = async () => {
  useConfirmBeforeDelete(
    async () => {
      await scanStore.setDefaultK(route.params.id)
      await scanStore.getScanK(route.params.id)
    },
...
<template>
  <PageDefaultLayoutTwo>
    ...
    <HeaderOne
      >QuickScan privé situatie {{ clientSingleStore.first_name_client }}
