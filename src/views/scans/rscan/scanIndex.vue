...
import ReportFormal from '@/presentations/scan/rscan/reportForm.vue'
import ReportInformal from '@/presentations/scan/rscan/reportInform.vue'
...
import editScan from '@/views/scans/rscan/editScan.vue'
...
const resetData = async () => {
  useConfirmBeforeDelete(
    async () => {
      await scanStore.setDefaultR(route.params.id)
      await scanStore.getScanR(route.params.id)
    },
...
<template>
  <PageDefaultLayoutTwo>
    ...
    <HeaderOne
      >QuickScan managementinformatie {{ clientSingleStore.first_name_client }}
