...
const saveExisting = async () => {
  ...
  await scanStore.changeScanA()
  if (scanStore.error === null) {
    // await teamSingleStore.setDefault()
    await scanStore.getScanA(route.params.id)
  ...
  >
    <div role="alert" class="container mx-auto w-11/12">
      ...
      >
        ...
        <DividerTable />
        <HeaderOne>Extra informatie ten behoeve van presentatie</HeaderOne>
        <div class="grid grid-cols-4 gap-4">
          <div>
            <HeaderTwo>Soort onderneming</HeaderTwo>
            <TextOne>
            Geef de ondernemingsvorm aan
            </TextOne>
          </div>
          <FormKit
            type="radio"
            label="Soort onderneming"
            v-model="scanStore.scanData.int_question_a"
            validation="required"
            validation-visibility="dirty"
            :options="[
              { label: 'BV Besloten Vennootschap', value: 'ke1' },
              { label: 'VOF Vennootschap onder Firma', value: 'ke2' },
              { label: 'EZ Eenmanszaak', value: 'ke3' }
            ]"
          />
        </div>
        <div class="grid grid-cols-4 gap-4" v-if="scanStore.scanData.int_question_a === 'ke1'">
          <div>
            <HeaderTwo>Aanbieding BV</HeaderTwo>
            <TextOne>
              Wat is uw aanbieding voor het samenstellen van een noodplan
              voor de BV? (bedrag in euro)
            </TextOne>
          </div>
          <FormKit
            type="text"
            ...
            validation="required"
            validation-visibility="dirty"
            v-model="scanStore.scanData.int_question_b"
          />
        </div>
        <div class="grid grid-cols-4 gap-4" v-if="scanStore.scanData.int_question_a === 'ke1'">
          <div>
            <HeaderTwo>Aanbieding PH</HeaderTwo>
            <TextOne>
              Wat is uw aanbieding voor het samenstellen van een noodplan
              voor de PH? (bedrag in euro)
            </TextOne>
          </div>
          <FormKit
            type="text"
            ...
            validation="required"
            validation-visibility="dirty"
            v-model="scanStore.scanData.int_question_c"
          />
        </div>
        <div class="grid grid-cols-4 gap-4" v-if="scanStore.scanData.int_question_a === 'ke2'">
          <div>
            <HeaderTwo>Aanbieding VOF</HeaderTwo>
            <TextOne>
              Wat is uw aanbieding voor het samenstellen van een noodplan
              voor de VOF? (bedrag in euro)
            </TextOne>
          </div>
          <FormKit
            type="text"
            ...
            validation="required"
            validation-visibility="dirty"
            v-model="scanStore.scanData.int_question_d"
          />
        </div>
        <div class="grid grid-cols-4 gap-4" v-if="scanStore.scanData.int_question_a === 'ke3'">
          <div>
            <HeaderTwo>Aanbieding EZ</HeaderTwo>
            <TextOne>
              Wat is uw aanbieding voor het samenstellen van een noodplan
              voor de EZ?(bedrag in euro)
            </TextOne>
          </div>
          <FormKit
            type="text"
            ...
            validation="required"
            validation-visibility="dirty"
            v-model="scanStore.scanData.int_question_e"
          />
        </div>
