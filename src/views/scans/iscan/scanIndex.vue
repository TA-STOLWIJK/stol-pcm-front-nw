...
import ReportFormal from '@/presentations/scan/iscan/reportFormal.vue'
import ReportInformal from '@/presentations/scan/iscan/reportInformal.vue'
...
import editScan from '@/views/scans/iscan/editScan.vue'
...
const resetData = async () => {
  useConfirmBeforeDelete(
    async () => {
      await scanStore.setDefaultI(route.params.id)
      await scanStore.getScanI(route.params.id)
    },
...
<template>
  <PageDefaultLayoutTwo>
    ...
    <HeaderOne
      >QuickScan cyber security {{ clientSingleStore.first_name_client }}
