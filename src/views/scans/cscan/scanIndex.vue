...
import ReportFormal from '@/presentations/scan/cscan/reportFormal.vue'
import ReportInformal from '@/presentations/scan/cscan/reportInformal.vue'
...
import editScan from '@/views/scans/cscan/editScan.vue'
...
const resetData = async () => {
  useConfirmBeforeDelete(
    async () => {
      await scanStore.setDefaultC(route.params.id)
      await scanStore.getScanC(route.params.id)
    },
...
<template>
  <PageDefaultLayoutTwo>
    ...
    <HeaderOne
      >QuickScan optimaal financieel pakket voor {{ clientSingleStore.first_name_client }}
