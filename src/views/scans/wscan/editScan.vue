...
const saveExisting = async () => {
  ...
  await scanStore.changeScanW()
  if (scanStore.error === null) {
    // await teamSingleStore.setDefault()
    await scanStore.getScanW(route.params.id)
  ...
  >
    <div role="alert" class="container mx-auto w-11/12">
      ...
      >
        ...
        <DividerTable />
        <HeaderOne>Uitschakelen onderdelen</HeaderOne>
        <div class="grid grid-cols-4 gap-4">
          <div>
            <HeaderTwo>Selecteren modules</HeaderTwo>
            <TextOne>
              Standaard zijn alle modules geselecteerd. Je kan een ervoor kiezen om een aantal
              modules niet aan te bieden aan een klant. Bijvoorbeeld als de klant geen bedrijfspand
              heeft. Deze modules komen dan ook niet terug in de verschillende rapportages.(PDF
              PPTX)</TextOne
            >
          </div>
        </div>
        ...
        <div class="grid grid-cols-1">
          <FormKit
            type="checkbox"
            label="HRM (niet uit te schakelen)"
            v-model="scanStore.scanData.vis_a"
            disabled
          />
          <FormKit
            type="checkbox"
            label="Duurzame inzet personeel"
            v-model="scanStore.scanData.vis_b"
          />
          <FormKit type="checkbox" label="Arbo en verzuim" v-model="scanStore.scanData.vis_c" />
          <FormKit type="checkbox" label="Pensioen" v-model="scanStore.scanData.vis_d" />
          <FormKit type="checkbox" label="Arbeidsrecht, loonheffing en sociale zekerheid" v-model="scanStore.scanData.vis_e" />
          <FormKit type="checkbox" label="Personeels- en salarisadministratie" v-model="scanStore.scanData.vis_f" />
        </div>
