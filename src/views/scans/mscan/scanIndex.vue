...
import ReportFormal from '@/presentations/scan/mscan/reportForm.vue'
import ReportInformal from '@/presentations/scan/mscan/reportInform.vue'
...
import editScan from '@/views/scans/mscan/editScan.vue'
...
const resetData = async () => {
  useConfirmBeforeDelete(
    async () => {
      await scanStore.setDefaultM(route.params.id)
      await scanStore.getScanM(route.params.id)
    },
...
<template>
  <PageDefaultLayoutTwo>
    ...
    <HeaderOne
      >QuickScan Groei {{ clientSingleStore.first_name_client }}
