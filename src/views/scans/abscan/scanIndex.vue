...
import ReportFormal from '@/presentations/scan/abscan/reportForm.vue'
import ReportInformal from '@/presentations/scan/abscan/reportInform.vue'
...
import editScan from '@/views/scans/abscan/editScan.vue'
...
const resetData = async () => {
  useConfirmBeforeDelete(
    async () => {
      await scanStore.setDefaultAB(route.params.id)
      await scanStore.getScanAB(route.params.id)
    },
...
<template>
  <PageDefaultLayoutTwo>
    ...
    <HeaderOne
      >QuickScan update {{ clientSingleStore.first_name_client }}
