...
import ReportInformal from '@/presentations/scan/escan/reportInformal.vue'
import ReportFormal from '@/presentations/scan/escan/reportFormal.vue'
...
import editScan from '@/views/scans/escan/editScan.vue'
...
const resetData = async () => {
  useConfirmBeforeDelete(
    async () => {
      await scanStore.setDefaultE(route.params.id)
      await scanStore.getScanE(route.params.id)
    },
...
<template>
  <PageDefaultLayoutTwo>
    ...
    <HeaderOne
      >QuickScan juridische aspecten {{ clientSingleStore.first_name_client }}
