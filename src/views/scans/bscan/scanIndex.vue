...
import ReportFormal from '@/presentations/scan/bscan/reportFormal.vue'
import ReportInformal from '@/presentations/scan/bscan/reportInformal.vue'
...
import editScan from '@/views/scans/bscan/editScan.vue'
...
const resetData = async () => {
  useConfirmBeforeDelete(
    async () => {
      await scanStore.setDefaultB(route.params.id)
      await scanStore.getScanB(route.params.id)
    },
...
<template>
  <PageDefaultLayoutTwo>
    ...
    <HeaderOne
      >QuickScan Familiestatuut voor {{ clientSingleStore.first_name_client }}
    ...
    <HeaderTwo class="mt-5"
      >Let op deze QuickScan kent nog een aantal andere variabelen deze zijn aan te passen bij de
      standaard opties</HeaderTwo
    >
    <DividerTable />
